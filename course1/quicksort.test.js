...
const testComparisons = (testCase, count) => () => {
  const input = readInput(`./assigment3TestCases/input_dgrcode_${testCase}.txt`);
  const lastIndex = input.length - 1;
  const [expected1, expected2, expected3] = readInput(`./assigment3TestCases/output_dgrcode_${testCase}.txt`);
